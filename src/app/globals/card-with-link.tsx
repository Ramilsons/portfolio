...
export default function CardWithLink({target, label, image, content}: Props) {
    ...
    return (
        <Link href={{hash: target}} className="relative group" title={`Clique para ir para a seção ${label}`}>
            <Image alt={alt} src={`/images/${image}`} width={'300'} height={'180'} className="cursor-pointer group-hover:scale-[1.1] w-[100%] min-h-[190px] bg-[#E9F7FF] rounded-[12px]" />
            <p className={`absolute text-center top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  group-hover:scale-[1.1] mr-[auto] ml-[auto] ${classText}`}>{content ? content : ''}</p>
        </Link>
    )
}